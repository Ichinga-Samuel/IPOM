...
def model(TPd, op=(1450, 109, 109, 218, 60), params=None):
    ...
    Lb4 = (2 * C4) + ((pi / 2) * (Dap + Drp)) + (((Dap - Drp) ** 2)) / (
                4 * C4)  # theoretical pitch length of the auger/screw press drive belt, in m
    ...
    Cs = pi * Dsf  # Circumference of screw flight, in m
    ...
    parameters = [DigesterParameters, SeparatorParameters, ScrewPressParameters, PulleyParameters, BeltParameters, PowerRequirement]
    res = []
    for p in params:
        for q in parameters:
            if p in q:
                res.append(round(q[p][0], 4)) if isinstance(q[p][0], (int, float)) else res.append(q[p][0])
                break
    return res


def compare(params):
    res = ([i, *model(i, params=params)] for i in range(280, 5651))
    with open('results.csv', 'w', newline='') as cw:
        rw = csv.writer(cw, delimiter=',')
        rw.writerow(['ThroughPut', *params])
        rw.writerows(res)
    print("Done")


compare(['Standard inner diameter of digester barrel'])
